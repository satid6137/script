...
$apiKey = $cfg['api_key'];        // ✔ API KEY จังหวัด (telemed_config)
...
// ------------------------------
// ส่งรายวัน (ถูกต้องที่สุด)
// ------------------------------

$results = [];   // เก็บผลลัพธ์รายวัน

foreach ($his as $row) {

    // ข้อมูลรายวัน
    $telemed_total = intval($row['telemed'] ?? 0);
    $telemed_b2b = intval($row['b2b'] ?? 0);
    $telemed_b2c = intval($row['b2c'] ?? 0);
    ...
    $payload = [
        ...
        'telemed_total' => $telemed_total,
        'telemed_b2b_count' => $telemed_b2b,
        'telemed_b2c_count' => $telemed_b2c,
        'health_rider_total' => $health_rider_total,
        'details' => $details
    ];
    ...
    // ✔ แก้ status ให้ตรงระบบใหม่
    $status = ($res['status'] === 'success') ? 'success' : 'fail';

    // ✔ บันทึก hos_code ลง log
    $stmt = $conn->prepare("
        INSERT INTO telemed_log (log_date, report_date, status, message, hos_code)
        VALUES (NOW(), ?, ?, ?, ?)
    ");
    $stmt->bind_param('ssss', $row['visit_date'], $status, $res['message'], $hosCode);
    $stmt->execute();
    $stmt->close();
